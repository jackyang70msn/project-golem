...
- **App Layer**：`apps/runtime/index.js`（核心入口）與 `apps/dashboard/plugin.js`（Dashboard 插件）。
- **相容入口**：根目錄 `index.js`、`dashboard.js` 為 shim，確保舊指令可用。
- **`src` 三層骨架**：`src/domain`、`src/application`、`src/infrastructure`（漸進遷移中）。
